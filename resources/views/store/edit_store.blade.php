...
<div class="container-fluid">
    <div class="d-flex justify-content-between main-wrapper">
        ...
        <a href="{{route('stores.index')}}">
            <button type="button" class="btn btn-primary btn-xs text-white">Store list</button>
        </a>
    </div>
    <div class="card">
        
        <div class="card-header card-block">
            <form action="{{route('stores.update',['store'=>$store->id])}}" method="POST" class="form-horizontal">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6"><br>
                        <label for="store_name" class="form-title form-control-label">Store Name</label>
                        <input name="store_name" type="text" id="store_name" placeholder="Store Name" class="form-control @error('store_name') is-invalid @enderror" value="{{$store->store_name ?? old('store_name')}}">
                        @error('store_name')
                    ...
                    <div class="col-md-6"><br>
                        ...
                        <input name="store_address" type="text" id="store_address" placeholder="Store address" class="form-control @error('store_address') is-invalid @enderror" value="{{$store->store_address ?? old('store_address')}}">
                        @error('store_address')
                            <div class="alert alert-danger">{{$message}}</div>
                        @enderror
                    </div><br>
                    <div class="col-md-6"><br>
                        ...
                        <input name="contact_person" type="text" id="contact_person" placeholder="Store contact_person" class="form-control @error('contact_person') is-invalid @enderror" value="{{$store->contact_person ?? old('contact_person')}}">
                    ...
                    <div class="col-md-6"><br>
                        ...
                        <input name="contact_no" type="text" id="contact_no" placeholder=" contact no" class="form-control @error('contact_no') is-invalid @enderror" value="{{$store->contact_no ?? old('contact_no')}}">
                        @error('contact_no')
                            <div class="alert alert-danger">{{$message}}</div>
                        @enderror
                    </div><br>
                    ...
                    <div class="col col-md-6">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-select form-control" aria-label="Default select example">
                                ...
                                <option value="1" @if ($store->status == 'Active'?'selected':'')@endif>Active</option>
                                <option value="0" @if ($store->status == 'InActive'?'selected':'')@endif>Inacive</option>
                            </select>
                            @error('status')
                                <div class="alert alert-danger">{{$message}}</div>
                            @enderror
